      ...
      <DropdownMenuContent className="w-80 glass border-primary/20 backdrop-blur-xl" align="end">
        ...
        <ScrollArea className="h-[400px] pr-4">
          <div className="p-2">
            <DropdownMenuGroup>
              <div className="mb-4">
                <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-2 px-2">Modern Collections</p>
                <div className="grid grid-cols-1 gap-2">
                  {THEMES.filter(t => t.category === "Modern").map((t) => (
                    <DropdownMenuItem
                      key={t.id}
                      onClick={() => setTheme(t.id)}
                      className={`flex flex-col items-start p-3 cursor-pointer rounded-xl transition-all ${
                        theme.id === t.id ? "bg-primary/20 border border-primary/30" : "hover:bg-muted/50 border border-transparent"
                      }`}
                    >
              ...
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-2 px-2">Pastel Collection</p>
                <div className="grid grid-cols-1 gap-2">
