...
const Technologies = () => {
  ...
  const { data: technologies = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const data = await api.get('/technologies/stats');
      return data.map((item: any): TechDetail => ({
        id: item.id,
        name: item.name,
        category_name: item.category_name,
        developer_count: item.developer_count,
        avg_proficiency: parseFloat(item.avg_proficiency),
        avg_years_used: parseFloat(item.avg_years_used),
      }));
    },
  });
